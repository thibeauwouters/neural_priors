...
fs_ticks = 46
fs_labels = 48
LABELPAD = 0.175      # Padding for axis labels in corner plots
...
SRC_BOX_LABEL_FONTSIZE = 64   # Font size for source labels (BNS/NSBH)
...
POP_BOX_LABEL_FONTSIZE = 64   # Font size for population labels (Uniform/Gaussian/Double Gaussian)
...
# ==============================================
# CONFIGURATION: Ranges scaling factor
# ==============================================
# Scale factor to expand computed ranges symmetrically around their center
# 1.0 = no scaling, 1.1 = 10% wider ranges, etc.
RANGES_SCALE_FACTOR = 1.1

# Custom ranges -- easier to control
CORNERPLOT_RANGES = {"bns_uniform": [(0.8, 2.2),
                                     (0.35, 1.0),
                                     (0.0, 1200.0),
                                     (0.0, 350.0),
                                     ],
                     "bns_gaussian": [(1.0, 1.35),
                                      (0.75, 1.0),
                                      (0.0, 2200.0),
                                      (0.0, 180.0),
                                     ],
                     "bns_double_gaussian": [(1.0, 1.8),
                                             (0.5, 1.0),
                                             (0.0, 2000.0),
                                             (0.0, 200.0),
                                             ],
                     "nsbh_uniform": [(1.25, 3.2),
                                      (0.15, 1.0),
                                      (0.0, 4000.0),
                                      ],
                     "nsbh_gaussian": [(1.4, 2.4),
                                       (0.125, 0.7),
                                       (0.0, 2500.0),
                                       ],
                     "nsbh_double_gaussian": [(1.25, 2.8),
                                              (0.20, 1.0),
                                              (0.0, 2200.0),
                                              ],
...
GRID_VERTICAL_LINE_1_X = 0.3175  # X position of first vertical line (after Uniform column)
GRID_VERTICAL_LINE_2_X = 0.655  # X position of second vertical line (after Gaussian column)
...
def get_ranges(data: np.ndarray,
    ...
    Applies RANGES_SCALE_FACTOR to expand ranges symmetrically around their center.
    ...
    # Apply scaling factor to all ranges
    return apply_ranges_scale_factor(ranges)


def apply_ranges_scale_factor(ranges: list) -> list:
    """
    Apply RANGES_SCALE_FACTOR to a list of ranges, expanding them symmetrically around their center.

    Args:
        ranges (list): List of (min, max) tuples for each parameter.

    Returns:
        list: List of scaled (min, max) tuples.
    """
    scaled_ranges = []
    for min_val, max_val in ranges:
        center = (min_val + max_val) / 2
        width = max_val - min_val
        new_width = width * RANGES_SCALE_FACTOR
        scaled_min = center - new_width / 2
        scaled_max = center + new_width / 2
        scaled_ranges.append((scaled_min, scaled_max))
    return scaled_ranges


def apply_physical_boundaries(ranges: list, keys: list) -> list:
    """
    Apply physical constraints to ranges based on parameter names.

    Constraints:
    - mass_ratio: max ≤ 1.0
    - lambda_tilde: min ≥ 0.0
    - delta_lambda_tilde: min ≥ 0.0
    - lambda_1, lambda_2: min ≥ 0.0

    Args:
        ranges (list): List of (min, max) tuples for each parameter.
        keys (list): List of parameter names corresponding to the ranges.

    Returns:
        list: List of ranges with physical constraints applied.
    """
    if len(ranges) != len(keys):
        raise ValueError(f"Length of ranges ({len(ranges)}) must match length of keys ({len(keys)})")

    constrained_ranges = []
    for (min_val, max_val), key in zip(ranges, keys):
        # Mass ratio must be ≤ 1
        if key == "mass_ratio":
            max_val = min(max_val, 1.0)

        # Tidal parameters must be ≥ 0
        if key in ["lambda_tilde", "delta_lambda_tilde", "lambda_1", "lambda_2"]:
            min_val = max(min_val, 0.0)

        constrained_ranges.append((min_val, max_val))

    return constrained_ranges


def get_ranges_across_datasets(datasets_dict: dict,
                                quantile: float=0.0001) -> list:
    """
    Get the ranges across multiple datasets, taking the widest range (smallest xmin, largest xmax).
    Applies RANGES_SCALE_FACTOR to expand ranges symmetrically around their center.

    This ensures that when multiple datasets are plotted on the same panel, the ranges
    accommodate all datasets.

    Args:
        datasets_dict (dict): Dictionary mapping dataset names to numpy arrays of shape (n_samples, n_parameters).
        quantile (float): Quantile to use for determining ranges (default: 0.01).

    Returns:
        list: A list of tuples containing the (min, max) values for each parameter across all datasets.
    """
    if not datasets_dict:
        raise ValueError("datasets_dict cannot be empty")

    # Get the number of parameters from the first dataset
    first_dataset = next(iter(datasets_dict.values()))
    n_params = first_dataset.shape[1]
    ...
    for i in range(n_params):
        ...
        # Collect min/max from each dataset
        for dataset in datasets_dict.values():
            min_val = np.quantile(dataset[:, i], quantile)
            max_val = np.quantile(dataset[:, i], 1-quantile)
            all_mins.append(min_val)
            all_maxs.append(max_val)

        # Take the widest range (smallest min, largest max)
...
def combined_plot_both_sources(convert_masses: bool = True,
    ...
    for row_idx, source_type in enumerate(source_types):
        ...
        for pop_idx, pop in enumerate(populations):
            ...
            # Apply physical boundaries to ranges
            print(f"Applying physical boundaries to ranges for {pop} ({source_type})")
            ranges = apply_physical_boundaries(ranges, keys)
