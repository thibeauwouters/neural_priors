"""
Publication-ready corner plots for gravitational wave spin parameter analysis.

This script creates corner plots focusing on spin-related parameters (chi_eff, chi_p)
alongside mass parameters for comparing different EOS constraints. It follows the 
same structure as money_plots.py but focuses on spin characterization.
"""
...
def calculate_spin_parameter_ranges(samples_dict: Dict[str, np.ndarray], 
                                   default_samples: Optional[np.ndarray],
                                   params_to_plot: List[str],
                                   quantile_low: float = 0.01,
                                   quantile_high: float = 0.99) -> List[tuple]:
    """
    Calculate parameter ranges with automatic spin parameter range calculation.
    
    For chi_eff and chi_p, concatenates all data and computes quantiles to get
    the widest reasonable range. For other parameters, uses standard range calculation.
    
    Args:
        samples_dict (Dict[str, np.ndarray]): Dictionary of samples for each EOS
        default_samples (Optional[np.ndarray]): Default run samples if available
        params_to_plot (List[str]): List of parameter names
        quantile_low (float): Lower quantile for spin parameter ranges
        quantile_high (float): Upper quantile for spin parameter ranges
    ...
    # Collect all datasets for analysis
    all_datasets = list(samples_dict.values())
    if default_samples is not None:
        all_datasets.append(default_samples)
    
    for i, param in enumerate(params_to_plot):
        if param in ['chi_eff', 'chi_p']:
            # For spin parameters, concatenate all data and use quantiles
            all_param_data = []
            for dataset in all_datasets:
                if dataset.shape[1] > i:  # Check if parameter index exists
                    param_data = dataset[:, i]
                    # Remove NaN values
                    param_data = param_data[~np.isnan(param_data)]
                    all_param_data.extend(param_data)
            
            if all_param_data:
                all_param_data = np.array(all_param_data)
                param_min = np.quantile(all_param_data, quantile_low)
                param_max = np.quantile(all_param_data, quantile_high)
                ranges.append((param_min, param_max))
                print(f"Auto-calculated {param} range from quantiles {quantile_low}-{quantile_high}: [{param_min:.3f}, {param_max:.3f}]")
            else:
                # Fallback to default physical ranges
                if param == 'chi_eff':
                    ranges.append((-1.0, 1.0))
                    print(f"Using default {param} range: [-1.0, 1.0]")
                elif param == 'chi_p':
                    ranges.append((0.0, 1.0))
                    print(f"Using default {param} range: [0.0, 1.0]")
        else:
            # For non-spin parameters, use standard min/max across all datasets
            param_mins = []
            param_maxs = []
            for dataset in all_datasets:
                if dataset.shape[1] > i:  # Check if parameter index exists
                    ...
                    param_data = param_data[~np.isnan(param_data)]
                    if len(param_data) > 0:
                        param_mins.append(np.min(param_data))
                        param_maxs.append(np.max(param_data))
            
            if param_mins and param_maxs:
                param_min = min(param_mins)
                param_max = max(param_maxs)
                # Add 5% padding for non-spin parameters
                param_range = param_max - param_min
                param_min -= 0.05 * param_range
                param_max += 0.05 * param_range
                ranges.append((param_min, param_max))
                print(f"Auto-calculated {param} range: [{param_min:.6f}, {param_max:.6f}]")
            else:
                # Fallback to None (corner will calculate automatically)
                ranges.append(None)
                print(f"Could not calculate range for {param}, using corner default")
    
    return ranges


def plot_spin_corner_fixed_population_varying_eos(gw_event: str,
                                                  population: str = "gaussian",
                                                  source_type: str = "bns",
                                                  base_path: str = "../final_results/",
                                                  output_dir: str = "./figures/spin_plots/",
    ...
    """
    Create corner plot for a GW event focusing on spin parameters with varying EOS samples.
    
    This function compares different EOS constraints while keeping the population
    prior fixed, with focus on spin characterization (chi_eff, chi_p) alongside
    mass parameters.
    ...
    Args:
        ...
        params_to_plot (List[str]): Parameters to plot (default: spin-focused set)
        ranges (List[tuple]): Custom ranges for each parameter
        normalization_keys (List[str]): Dataset keys for histogram normalization
    ...
    # Set default parameters - focus on spin parameters
    if eos_samples is None:
        eos_samples = ["radio", "radio_chiEFT", "radio_NICER"]
    
    if params_to_plot is None:
        params_to_plot = ["chirp_mass", "mass_ratio", "chi_eff", "chi_p"]
    
    print(f"Creating {gw_event} spin corner plot for population: {population}, source: {source_type}")
    ...
    # Compute percentage of samples with negative delta lambda tilde for BNS (if relevant)
    ...
    # Handle NSBH lambda plotting if needed (though we're focusing on spin)
    ...
    try:
        ...
        if bf_dict:
            ...
            for i, (eos_name, _) in enumerate(sorted_bf):
                zorder_dict[eos_name] = len(sorted_bf) - i
        else:
    ...
    if ranges is not None:
        ...
        # Calculate ranges automatically with special handling for spin parameters
        print("Calculating ranges automatically...")
        ranges = calculate_spin_parameter_ranges(samples_dict, default_samples, params_to_plot)
        ...
        if verbose:
            print(f"Final calculated ranges: {ranges}")
    
    # Handle mixed custom/auto ranges by replacing None values with auto-calculated ranges
    if ranges is not None:
        final_ranges = []
        for i, range_val in enumerate(ranges):
            if range_val is None:
                # Auto-calculate this parameter's range
                if i < len(params_to_plot):
                    param = params_to_plot[i] if len(params_to_plot) == len(ranges) else labels[i]
                    auto_ranges = calculate_spin_parameter_ranges(samples_dict, default_samples, [param])
                    final_ranges.append(auto_ranges[0])
                else:
                    final_ranges.append(None)
            else:
                final_ranges.append(range_val)
        ranges = final_ranges
    ...
    # Save figure
    output_filename = f"{gw_event}_spin_corner_{population}_{source_type}.pdf"
...
def plot_gw170817_spin_corner() -> str:
    """
    Convenience function for creating GW170817 spin corner plot.
    """
    # Set defaults for GW170817 spin analysis - all ranges auto-calculated
    gw170817_spin_defaults = {
    ...
    # Gaussian
    path = plot_spin_corner_fixed_population_varying_eos(
        gw_event="GW170817",
        population="gaussian",
        source_type="bns",
    ...
    # Double Gaussian
    gw170817_spin_defaults['normalization_keys'] = ["radio_chiEFT",
                                                    "radio_NICER",
                                                    "radio_chiEFT",
                                                    "radio_chiEFT"
                                                    ]
    ...
    # Uniform
    gw170817_spin_defaults['normalization_keys'] = ["radio",
                                                    "radio_NICER",
                                                    "radio_chiEFT", 
                                                    "radio"
                                                    ]
...
def plot_gw190425_spin_corner() -> str:
    """
    Convenience function for creating GW190425 spin corner plot.
    """
    ...
    # Set defaults for GW190425 spin analysis - all ranges auto-calculated
    gw190425_spin_defaults = {
    ...
    # Double Gaussian  
    gw190425_spin_defaults['normalization_keys'] = ["radio_chiEFT",
                                                    "radio_NICER",
                                                    "radio_NICER",
                                                    "radio_NICER"
                                                    ]
...
def plot_gw230529_spin_corner() -> str:
    """
    Convenience function for creating GW230529 spin corner plot.
    """
    
    # Set defaults for GW230529 spin analysis - all ranges auto-calculated
    gw230529_spin_defaults = {
    ...
    # Double Gaussian
    gw230529_spin_defaults['normalization_keys'] = ["radio",
                                                    "radio_NICER",
                                                    "radio_NICER",
                                                    "radio_NICER"]
    path = plot_spin_corner_fixed_population_varying_eos(
    ...
    # Uniform
    gw230529_spin_defaults['normalization_keys'] = ["radio_NICER",
                                                    "radio_chiEFT",
                                                    "radio_chiEFT",
                                                    "radio_NICER"]
    path = plot_spin_corner_fixed_population_varying_eos(
...
def main():
    plot_gw170817_spin_corner()
    plot_gw190425_spin_corner()
    plot_gw230529_spin_corner()
