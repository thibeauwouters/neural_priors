"""
Auxiliary figures for Inkscape schematics - Presentation version.
Creates simple, clean figures that can be imported into Inkscape for method visualization.
This version has the p_NF bracket on the right-hand side instead of at the bottom.
"""
...
# Right-side curly bracket parameters (modified from bottom bracket)
bracket_y_top = 0.88                  # Top edge of bracket (figure coordinates 0-1)
bracket_y_bottom = 0.08               # Bottom edge of bracket (figure coordinates 0-1)
bracket_x = 0.96                      # X position for bracket left edge (close to right side)
bracket_width = 0.025                 # Depth of the rightward bulge (horizontal extent)
bracket_text_x = 1.01                 # X position for text to the right of bracket
bracket_flat_length = 0.38            # Length of flat vertical sections (fraction of total height)
fs_bracket_text = 50                  # Font size for p_NF text
...
def create_mass_distributions(
    ...
    This presentation version has the p_NF bracket on the right side instead of at the bottom.
    ...
    # Adjust right margin to leave space for the right-side bracket
    gs = GridSpec(6, 3, figure=fig, hspace=HSPACE_POPULATION, wspace=WSPACE_COLUMNS,
                  ...
                  left=0.02, right=0.92)  # Reduced right to leave space for bracket
    ...
    # ===== Add curly bracket on the right side spanning all rows =====
    from matplotlib.patches import FancyBboxPatch
    import matplotlib.patches as mpatches

    # Create curly bracket using annotation
    # We'll draw it as a vertical bracket on the right side
    ax_bracket = fig.add_axes([0, 0, 1, 1], facecolor='none')
    ax_bracket.set_xlim(0, 1)
    ax_bracket.set_ylim(0, 1)
    ax_bracket.axis('off')

    # Manual curly bracket drawing using path (vertical version)
    from matplotlib.path import Path as MplPath
    import matplotlib.patches as patches

    # Define curly bracket path with flat top/bottom and sharp rightward bulge
    bracket_center = (bracket_y_top + bracket_y_bottom) / 2
    total_height = bracket_y_top - bracket_y_bottom

    # Define turning points
    top_turn = bracket_y_top - bracket_flat_length * total_height
    bottom_turn = bracket_y_bottom + bracket_flat_length * total_height

    # Curly bracket with sharp inflection: flat vertical sections, sharp rightward bulge
    verts = [
        (bracket_x, bracket_y_top),  # Start (top)
        (bracket_x, top_turn),  # Flat section top
        (bracket_x, top_turn - 0.01),  # Control point for sharp turn
        (bracket_x + bracket_width, bracket_center + 0.01),  # Control point approaching center
        (bracket_x + bracket_width, bracket_center),  # Center right point (bulge)
        (bracket_x + bracket_width, bracket_center - 0.01),  # Control point leaving center
        (bracket_x, bottom_turn + 0.01),  # Control point for sharp turn
        (bracket_x, bottom_turn),  # Flat section bottom
        (bracket_x, bracket_y_bottom),  # End (bottom)
    ]

    codes = [
        MplPath.MOVETO,
        MplPath.LINETO,  # Flat top section
        MplPath.CURVE4,  # Sharp turn right
        MplPath.CURVE4,
        MplPath.CURVE4,
        MplPath.CURVE4,  # Sharp turn left
        MplPath.CURVE4,
        MplPath.LINETO,  # Flat bottom section
        MplPath.LINETO,
    ]
    ...
    # Add text to the right of the bracket (rotated vertically, mirrored to read top to bottom)
    fig.text(bracket_text_x, bracket_center,
             r'$p_{\rm{NF}}(m_1^{\rm{src}}, m_2^{\rm{src}}, \Lambda_1, \Lambda_2 | \mathcal{H})$',
             ha='left', va='center', fontsize=fs_bracket_text,
             transform=fig.transFigure, rotation=90)

    if save:
        output_path = OUTPUT_DIR / "Figure1_presentation.pdf"
